...
| ----------------------- | -------------------------------------------------------------- |
| **Sintaxe**             | `reply(texto)`                                                 |
| **Parâmetros**          | `texto` _(string)_ — A mensagem que você deseja enviar.        |
...
| **Parâmetros**          | `texto` _(string)_ — A mensagem a ser enviada. |
...
| **Parâmetros** | `link` _(string)_ — URL pública da imagem (JPEG, PNG). |
...
| **Parâmetros**          | `link` _(string)_ — URL pública da imagem. `texto` _(string)_ — Legenda que aparecerá abaixo da imagem. |
...
| **Parâmetros** | `link` _(string)_ — URL pública do vídeo (MP4). |
...
| **Parâmetros**          | `link` _(string)_ — URL pública do vídeo. `texto` _(string)_ — Legenda que aparecerá abaixo do vídeo. |
| **Suporte LID/Menções** | ✅ Sim                                                                                                |
...
| **Parâmetros** | `link` _(string)_ — URL pública do áudio (MP3). |

**Explicação:** Envia um arquivo de áudio com `ptt: true` _(Push To Talk)_, fazendo com que o WhatsApp o reproduza como mensagem de voz gravada.
...
| **Parâmetros** | `emoji` _(string)_ — O emoji usado como reação (ex: `"✅"`, `"❤️"`). |
...
| **Parâmetros** | `subaru` — objeto da conexão (já disponível no escopo). `from` — ID do chat (já disponível como `from`). `fonte` — Buffer, URL ou caminho do arquivo de imagem. `quoted` — a mensagem citada (use `info` para a mensagem atual). `options` _(opcional)_ — objeto com `packname` e `author`. |
...
| **Parâmetros** | `teks` _(string)_ — Texto a enviar (pode conter `@` para menções). `membrosGrupo` _(array)_ — Lista de JIDs dos membros. `id` _(string \| null)_ — ID opcional para controle interno (geralmente `null`). |
