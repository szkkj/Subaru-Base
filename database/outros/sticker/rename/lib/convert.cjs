...
exports.webpToMedia = function () {
  return new Promise(async (resolve, reject) => {
    ...
    this.exec(
      `convert ${this.file} -trim +repage -background transparent -flatten ${img}`,
...
exports.imageEdit = function (file) {
  return this.download(file, this.diretory + "merge_" + Date.now()).then(
    (res) => {
      return new Promise(async (resolve, reject) => {
        if (!this.file) return reject("Sem arquivo para continuar.");
        if (res.ext !== "png") {
        ...
        ffmpeg(this.file)
          ...
          .on("end", (end) => {
            ...
            this.exec(
              `ffmpeg -hide_banner -i ${cropImg} -i ${res.file} -filter_complex "[0]scale=400:400[ava];[1]alphaextract[alfa];[ava][alfa]alphamerge" ${dateImg}`,
              async (err, stdout, stderr) => {
                this.deleteFile(res.file);
