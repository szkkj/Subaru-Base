...
module.exports = {
  ...
  removeExtFile(file) {
    ...
    file = file
      .split(".")
      .filter((i) => !FileType.extensions.has(i))
      .join(".");
  ...
  mkDir(paste) {
    ...
    if (folderInfo) {
      if (!folderInfo?.isDirectory()) {
        throw new Error(
          `Encontrou algo que não é um diretório em ${paste}, exclua-o ou especifique um local diferente.`,
        );
      }
    } else {
    ...
    return new Promise((resolve, reject) => {
      ...
      (url.startsWith("https://") ? https.get : http.get)(url, (response) => {
        if (response.statusCode !== 200) {
          reject(
            `Falha na requisição. Código de status: ${response.statusCode}`,
          );
          return;
        }
  ...
  async fetchBuffer(url, options = {}) {
    ...
    return fetch(url, typeof options == "object" ? options : {})
      .then(async (res) => {
        ...
        if (!Buffer.isBuffer(buffer))
          return Promise.reject("Sem resultado de Buffer.");
  ...
  getBuffer(url, options = []) {
    ...
    return this.promise(
      options.filter((i) => i).map((config) => this.fetchBuffer(url, config)),
      "any",
    );
  },
  async download(file, fileName) {
    ...
    if (this.existsSync(file)) {
      ...
      return Promise.reject(
        "Não foi detectado nenhum tipo de arquivo conhecido.",
      );
    }
  },
};
