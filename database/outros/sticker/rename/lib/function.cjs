...
exports.checkfile = function ({ file, ext }) {
  return new Promise((resolve, reject) => {
    this.exec(
      `ffprobe -v error -select_streams v:0 -show_entries stream=width,height,duration -of csv=p=0:s=x ${file}`,
      (err, stdout) => {
...
exports.promise = async function (array, type = "settled") {
  ...
  if (!array.length)
    return Promise.reject("Necessário ter algo dentro da array.");
  ...
  switch (type) {
    case "settled":
      ...
      return results
        ? Promise.resolve(results.value)
        : Promise.reject("Nenhuma data foi encontrada.");
    case "any":
