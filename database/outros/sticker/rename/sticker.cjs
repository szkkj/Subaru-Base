...
exports.Sticker = class Sticker {
  ...
  clear() {
    for (const f of ["file", "ext", "time", "width", "height"]) {
      delete this[f];
    }
    this.arrayFiles = [];
  }
  ...
  async start() {
    ...
    const read = this.arrayFiles.map((file, index) => {
      ...
      return this.download(file, this.diretory, Date.now()).then(async (v) => {
        ...
        try {
          if (this.options.transparent && this.options.transparent.length) {
    ...
    if (!read.length)
      return Promise.reject(
        "Não foi adicionado nenhum arquivo/buffer/url no cache.",
      );
