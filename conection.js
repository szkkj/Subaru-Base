...
const { default: makeWASocket, DisconnectReason, useMultiFileAuthState, fetchLatestBaileysVersion, isJidBroadcast, isJidStatusBroadcast, getContentType, makeCacheableSignalKeyStore } = baileysPkg;
...
const phoneNumber_raw = await new Promise(resolve => rl.once('line', resolve));
...
reconnectAttempts++;
console.log(`Reconectando em ${reconnectDelay / 1000}s... (tentativa ${reconnectAttempts})`);
...
if (String(err).includes('SenderKeyRecord') || String(err).includes('decrypt') || String(err).includes('no session')) {
...
export { startConnection, getGroupMetadataSafe, getGroupConfig, messageCache, groupMetadataCache };
