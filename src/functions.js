...
const groupConfigCache = new LRU({ maxItems: 100, maxAge: 600 });
...
async function gerarlinkUploadCatbox(buffer, filename) {
  ...
  if (!link || !link.startsWith("http"))
    throw new Error("Erro ao enviar para Catbox");
  return link.trim();
}
...
async function checarVersao(reply2, subaru, from) {
  try {
    const res = await fetch(
      `https://raikken.com.br/api/subaru/versao?versao=${baseVersion}`,
    );
...
async function atualizarBot(subaru, seloSz, from) {
  ...
  await subaru.sendMessage(from, {
    text: "Aguarde, estarei fazendo o backup e enviando no PV do dono",
  });
...
const getFamiliaData = async (usuarioId) => {
  try {
    const res = await fetch(
      `${baseRaikken}/api/familia/arvore/${usuarioId}?apikey=${RaikkenKey}`,
    );
    if (res.status === 404) {
      return null;
    }
...
function getGroupConfig(id) {
  ...
  if (!fs.existsSync(path.join(__dirname, `../database/grupos/${id}.json`)))
    return null;
