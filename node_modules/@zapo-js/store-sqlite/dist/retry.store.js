...
exports.WaRetrySqliteStore = void 0;
...
class WaRetrySqliteStore extends BaseSqliteStore_1.BaseSqliteStore {
    ...
    async getOutboundRequesterStatus(messageId, requesterDeviceJid) {
        ...
        const row = db.get(`SELECT requesters_json
             FROM retry_outbound_messages
             WHERE session_id = ? AND message_id = ?
             LIMIT 1`, [this.options.sessionId, messageId]);
        if (!row) {
            return null;
        }
        const requestersJson = (0, util_1.asOptionalString)(row.requesters_json, 'retry_outbound_messages.requesters_json');
        if (!requestersJson) {
            return null;
        }
    ...
    async getOutboundMessage(messageId) {
        ...
        return {
            messageId: (0, util_1.asString)(row.message_id, 'retry_outbound_messages.message_id'),
            toJid: (0, util_1.asString)(row.to_jid, 'retry_outbound_messages.to_jid'),
            ...
            replayMode: (0, util_1.asString)(row.replay_mode, 'retry_outbound_messages.replay_mode'),
            replayPayload: (0, util_1.asBytes)(row.replay_payload, 'retry_outbound_messages.replay_payload'),
            state: (0, util_1.asString)(row.state, 'retry_outbound_messages.state'),
            updatedAtMs: (0, util_1.asNumber)(row.updated_at_ms, 'retry_outbound_messages.updated_at_ms'),
            expiresAtMs: (0, util_1.asNumber)(row.expires_at_ms, 'retry_outbound_messages.expires_at_ms')
        };
    }
    ...
    async markOutboundRequesterDelivered(messageId, requesterDeviceJid, updatedAtMs, expiresAtMs) {
        await this.withTransaction((db) => {
            ...
            db.run(`UPDATE retry_outbound_messages
                 ...
                 WHERE session_id = ? AND message_id = ?`, [nextRequestersJson, updatedAtMs, expiresAtMs, this.options.sessionId, messageId]);
        });
    }
    async incrementInboundCounter(messageId, requesterJid, _updatedAtMs, expiresAtMs) {
        const db = await this.getConnection();
        const row = db.get(`INSERT INTO retry_inbound_counters (
                session_id,
                message_id,
                requester_jid,
                retry_count,
            ...
            ON CONFLICT(session_id, message_id, requester_jid) DO UPDATE SET
                ...
                expires_at_ms=excluded.expires_at_ms
            RETURNING retry_count`, [this.options.sessionId, messageId, requesterJid, expiresAtMs]);
        if (!row) {
            return 1;
        }
        return (0, util_1.asNumber)(row.retry_count, 'retry_inbound_counters.retry_count');
    }
    async cleanupExpired(nowMs) {
        return this.withTransaction((db) => {
            ...
            const outboundCount = outboundCountRow
                ? (0, util_1.asNumber)(outboundCountRow.total, 'retry_outbound_messages.changes')
                : 0;
            ...
            const inboundCount = inboundCountRow
                ? (0, util_1.asNumber)(inboundCountRow.total, 'retry_inbound_counters.changes')
                : 0;
