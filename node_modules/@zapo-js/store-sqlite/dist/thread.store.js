"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.WaThreadSqliteStore = void 0;
const util_1 = require("zapo-js/util");
const BaseSqliteStore_1 = require("./BaseSqliteStore");
const THREAD_COLUMNS = 'jid, name, unread_count, archived, pinned, mute_end_ms, marked_as_unread, ephemeral_expiration, ephemeral_setting_timestamp';
function decodeThreadRow(row) {
    return {
        jid: (0, util_1.asString)(row.jid, 'mailbox_threads.jid'),
        name: (0, util_1.asOptionalString)(row.name, 'mailbox_threads.name'),
        unreadCount: (0, util_1.asOptionalNumber)(row.unread_count, 'mailbox_threads.unread_count'),
        archived: (0, util_1.toBoolOrUndef)(row.archived),
        pinned: (0, util_1.asOptionalNumber)(row.pinned, 'mailbox_threads.pinned'),
        muteEndMs: (0, util_1.asOptionalNumber)(row.mute_end_ms, 'mailbox_threads.mute_end_ms'),
        markedAsUnread: (0, util_1.toBoolOrUndef)(row.marked_as_unread),
        ephemeralExpiration: (0, util_1.asOptionalNumber)(row.ephemeral_expiration, 'mailbox_threads.ephemeral_expiration'),
        ephemeralSettingTimestamp: (0, util_1.asOptionalNumber)(row.ephemeral_setting_timestamp, 'mailbox_threads.ephemeral_setting_timestamp')
    };
}
class WaThreadSqliteStore extends BaseSqliteStore_1.BaseSqliteStore {
    constructor(options) {
        super(options, ['mailbox']);
    }
    async upsert(record) {
        const db = await this.getConnection();
        this.upsertThreadRow(db, record);
    }
    async upsertBatch(records) {
        if (records.length === 0) {
            return;
        }
        await this.withTransaction((db) => {
            for (const record of records) {
                this.upsertThreadRow(db, record);
            }
        });
    }
    async getByJid(jid) {
        const db = await this.getConnection();
        const row = db.get(`SELECT ${THREAD_COLUMNS}
             FROM mailbox_threads
             WHERE session_id = ? AND jid = ?`, [this.options.sessionId, jid]);
        return row ? decodeThreadRow(row) : null;
    }
    async list(limit) {
        const db = await this.getConnection();
        const rows = db.all(`SELECT ${THREAD_COLUMNS}
             FROM mailbox_threads
             WHERE session_id = ?
             LIMIT ?`, [this.options.sessionId, (0, util_1.normalizeQueryLimit)(limit, 100)]);
        const threads = new Array(rows.length);
        for (let index = 0; index < rows.length; index += 1) {
            threads[index] = decodeThreadRow(rows[index]);
        }
        return threads;
    }
    async deleteByJid(jid) {
        const db = await this.getConnection();
        db.run(`DELETE FROM mailbox_threads
             WHERE session_id = ? AND jid = ?`, [this.options.sessionId, jid]);
        const row = db.get('SELECT changes() AS total', []);
        return row ? Number(row.total) : 0;
    }
    async clear() {
        const db = await this.getConnection();
        db.run('DELETE FROM mailbox_threads WHERE session_id = ?', [this.options.sessionId]);
    }
    upsertThreadRow(db, record) {
        db.run(`INSERT INTO mailbox_threads (
                session_id, jid, name, unread_count, archived, pinned,
                mute_end_ms, marked_as_unread, ephemeral_expiration, ephemeral_setting_timestamp
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT(session_id, jid) DO UPDATE SET
                name=COALESCE(excluded.name, mailbox_threads.name),
                unread_count=COALESCE(excluded.unread_count, mailbox_threads.unread_count),
                archived=COALESCE(excluded.archived, mailbox_threads.archived),
                pinned=COALESCE(excluded.pinned, mailbox_threads.pinned),
                mute_end_ms=COALESCE(excluded.mute_end_ms, mailbox_threads.mute_end_ms),
                marked_as_unread=COALESCE(excluded.marked_as_unread, mailbox_threads.marked_as_unread),
                ephemeral_expiration=COALESCE(excluded.ephemeral_expiration, mailbox_threads.ephemeral_expiration),
                ephemeral_setting_timestamp=COALESCE(excluded.ephemeral_setting_timestamp, mailbox_threads.ephemeral_setting_timestamp)`, [
            this.options.sessionId,
            record.jid,
            record.name ?? null,
            record.unreadCount ?? null,
            record.archived === undefined ? null : record.archived ? 1 : 0,
            record.pinned ?? null,
            record.muteEndMs ?? null,
            record.markedAsUnread === undefined ? null : record.markedAsUnread ? 1 : 0,
            record.ephemeralExpiration ?? null,
            record.ephemeralSettingTimestamp ?? null
        ]);
    }
}
exports.WaThreadSqliteStore = WaThreadSqliteStore;
