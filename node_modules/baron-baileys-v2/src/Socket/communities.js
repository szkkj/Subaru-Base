...
const makeCommunitiesSocket = (config) => {
  ...
  const communityFetchAllParticipating = async () => {
    ...
    const data = {};
    const communitiesChild = (0, WABinary_1.getBinaryNodeChild)(
      result,
      "communities",
    );
    if (communitiesChild) {
      const communities = (0, WABinary_1.getBinaryNodeChildren)(
        communitiesChild,
        "community",
      );
  ...
  return {
    ...
    communityFetchLinkedGroups: async (jid) => {
      ...
      const subGroupsNode = (0, WABinary_1.getBinaryNodeChild)(
        result,
        "sub_groups",
      );
      if (subGroupsNode) {
        const groupNodes = (0, WABinary_1.getBinaryNodeChildren)(
          subGroupsNode,
          "group",
        );
        for (const groupNode of groupNodes) {
          linkedGroupsData.push({
            id: groupNode.attrs.id
              ? (0, WABinary_1.jidEncode)(groupNode.attrs.id, "g.us")
              : undefined,
            ...
            owner: groupNode.attrs.creator
              ? (0, WABinary_1.jidNormalizedUser)(groupNode.attrs.creator)
              : undefined,
    ...
    communityAcceptInviteV4: ev.createBufferedFunction(
      async (key, inviteMessage) => {
        ...
        if (key.id) {
          ...
          inviteMessage =
            index_js_1.proto.Message.GroupInviteMessage.fromObject(
              inviteMessage,
            );
...
const extractCommunityMetadata = (result) => {
  ...
  const descChild = (0, WABinary_1.getBinaryNodeChild)(
    community,
    "description",
  );
  let desc;
  ...
  const communityId = (
    (_a = community.attrs.id) === null || _a === void 0
      ? void 0
      : _a.includes("@")
  )
    ? community.attrs.id
  ...
  const eph =
    (_b = (0, WABinary_1.getBinaryNodeChild)(community, "ephemeral")) ===
      null || _b === void 0
      ? void 0
      : _b.attrs.expiration;
  const memberAddMode =
    (0, WABinary_1.getBinaryNodeChildString)(community, "member_add_mode") ===
    "all_member_add";
  const metadata = {
    id: communityId,
    ...
    size: (0, WABinary_1.getBinaryNodeChildren)(community, "participant")
      .length,
    ...
    descId,
    linkedParent:
      ((_c = (0, WABinary_1.getBinaryNodeChild)(community, "linked_parent")) ===
        null || _c === void 0
        ? void 0
        : _c.attrs.jid) || undefined,
    ...
    addressingMode: (0, WABinary_1.getBinaryNodeChildString)(
      community,
      "addressing_mode",
    ),
  };
