...
const makeMessagesSocket = (config) => {
  ...
  const profilePictureUrl = async (jid) => {
    if (WABinary_1.isJidNewsletter(jid)) {
      let node = await newsletterWMexQuery(
        undefined,
      ...
      let result = WABinary_1.getBinaryNodeChild(
        node,
        "result",
      )?.content?.toString();
      ...
      return Utils_1.getUrlFromDirectPath(
      ...
      if (useCache) {
        ...
        if (devices) {
          ...
          const devicesWithWire = devices.map((d) => ({
            ...d,
            wireJid: isLidJid
              ? WABinary_1.jidEncode(d.user, "lid", d.device)
              : WABinary_1.jidEncode(d.user, "s.whatsapp.net", d.device),
          }));
          ...
          if (
            ...
            const lidDeviceJid =
              decoded.device !== undefined
                ? `${WABinary_1.jidDecode(lidForPN).user}:${decoded.device}@lid`
                : lidForPN;
      ...
      ([user, userDevices]) =>
        encryptionMutex.mutex(user, async () => {
          ...
          const getEncryptionJid = async (wireJid) => {
            if (!WABinary_1.isPnUser(wireJid)) return wireJid;

            try {
              const lidForPN =
                await signalRepository.lidMapping.getLIDForPN(wireJid);
              ...
              const lidWithDevice = WABinary_1.jidEncode(
                lidDecoded?.user,
                "lid",
                deviceId,
              );
    ...
    await authState.keys.transaction(async () => {
      ...
      if (isGroupOrStatus && !isRetryResend) {
        ...
        if (!isRetryResend) {
          ...
          if (user !== ownUser) {
            ...
            const ownUserForAddressing =
              isLid && meLid
                ? WABinary_1.jidDecode(meLid).user
                : WABinary_1.jidDecode(meId).user;
            devices.push({
              user: ownUserForAddressing,
              device: 0,
              wireJid: WABinary_1.jidEncode(
          ...
          if (isRetryResend) {
            const isParticipantLid = WABinary_1.isLidUser(participant.jid);
            const isMe = WABinary_1.areJidsSameUser(
              participant.jid,
              isParticipantLid ? meLid : meId,
            );

            const encodedMessageToSend = isMe
              ? Utils_1.encodeWAMessage({
                  deviceSentMessage: {
                    destinationJid,
                    message,
                  },
                })
              : Utils_1.encodeWAMessage(message);
            ...
            logger.debug(
              {
                ...
                devices: devices.map(
                  (d) =>
                    `${d.user}:${d.device}@${WABinary_1.jidDecode(d.wireJid)?.server}`,
                ),
              },
        ...
        if (meJids.length > 0 || otherJids.length > 0) {
          extraAttrs["phash"] = Utils_1.generateParticipantHashV2([
            ...meJids,
            ...otherJids,
          ]);
        }
  ...
  const getEphemeralGroup = (jid) => {
    ...
    return groups_1
      ...
      .then(
        (metadata) =>
          WABinary_1.getBinaryNodeChild(metadata, "ephemeral")?.attrs
            ?.expiration || 0,
      );
  };

  const waUploadToServer = Utils_1.getWAUploadToServer(
    config,
    refreshMediaConn,
  );

  const waitForMsgMediaUpdate = Utils_1.bindWaitForEvent(
    ev,
  ...
  return {
    ...
    updateMediaMessage: async (message) => {
      ...
      const meId = authState.creds.me.id;
      const node = await Utils_1.encryptMediaRetryRequest(
        message.key,
        mediaKey,
        meId,
      );
        ...
        waitForMsgMediaUpdate(async (update) => {
          ...
          if (result) {
            if (result.error) {
              ...
              try {
                const media = await Utils_1.decryptMediaRetryData(
                  result.media,
                  mediaKey,
                  result.key.id,
                );
                ...
                if (
                  ...
                  throw new boom_1.Boom(
                    `Media re-upload failed by device (${resultStr})`,
                    {
                      data: media,
                      statusCode:
                        Utils_1.getStatusCodeForMediaRetry(media.result) || 404,
                    },
                  );
                }
    ...
    sendStatusMentions: async (content, jids = []) => {
      ...
      let mediaHandle;
      try {
        msg = await Utils_1.generateWAMessage(
    ...
    sendMessage: async (jid, content, options = {}) => {
      ...
      if (
        ...
        if (isDelete || isKeep) {
          ...
          if (
            (WABinary_1.isJidGroup(content.delete?.remoteJid) &&
