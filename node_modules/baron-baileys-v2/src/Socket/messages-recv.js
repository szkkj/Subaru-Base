...
const makeMessagesRecvSocket = (config) => {
  ...
  const handleGroupNotification = (participant, child, msg, mode) => {
    let participantJid =
      mode === "lid"
        ? WABinary_1.getBinaryNodeChild(child, "participant")?.attrs
            ?.phone_number
        : WABinary_1.getBinaryNodeChild(child, "participant")?.attrs?.jid ||
          participant;
    ...
    switch (child.tag) {
      ...
      case "created_membership_requests":
        participantJid =
          mode === "lid"
            ? WABinary_1.getBinaryNodeChild(child, "requested_user")?.attrs
                ?.phone_number
            : WABinary_1.getBinaryNodeChild(child, "requested_user")?.attrs
                ?.jid || participant;
  ...
  const handlePrivacyTokenNotification = async (node) => {
    const tokensNode = WABinary_1.getBinaryNodeChild(node, "tokens");
    const from = WABinary_1.jidNormalizedUser(node.attrs.from);

    if (!tokensNode) return;

    const tokenNodes = WABinary_1.getBinaryNodeChildren(tokensNode, "token");
  ...
  const handleMessage = async (node) => {
    ...
    if (!!alt) {
      ...
      if (altServer === "lid") {
        if (
          typeof (await signalRepository.lidMapping.getPNForLID(alt)) ===
          "string"
        ) {
        ...
        if (
          typeof (await signalRepository.lidMapping.getLIDForPN(alt)) ===
          "string"
        ) {
          await signalRepository.lidMapping.storeLIDPNMappings([
        ...
        processingMutex.mutex(async () => {
          ...
          if (
            ...
            if (category === "peer") {
              ...
              if (
                WABinary_1.isLidUser(msg.key.remoteJid) ||
                WABinary_1.isLidUser(msg.key.remoteJidAlt)
              ) {
