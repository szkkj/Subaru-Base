...
exports.mobileRegisterAbprop = mobileRegisterAbprop;
exports.mobileRegisterSecurity = mobileRegisterSecurity;
exports.mobileRegisterClientLog = mobileRegisterClientLog;
exports.mobileRegisterConsent = mobileRegisterConsent;
...
const makeRegistrationSocket = (config) => {
  ...
  const requestRegistrationAbprop = async (registrationOptions) => {
    registrationOptions = registrationOptions || config.auth.creds.registration;
    if (!validRegistrationOptions(registrationOptions)) {
      throw new Error("Invalid registration options");
    }
    sock.authState.creds.registration = registrationOptions;
    sock.ev.emit("creds.update", sock.authState.creds);
    return mobileRegisterAbprop(
      { ...config.auth.creds, ...registrationOptions },
      config.options,
    );
  };

  const requestRegistrationSecurity = async (registrationOptions) => {
    registrationOptions = registrationOptions || config.auth.creds.registration;
    if (!validRegistrationOptions(registrationOptions)) {
      throw new Error("Invalid registration options");
    }
    sock.authState.creds.registration = registrationOptions;
    sock.ev.emit("creds.update", sock.authState.creds);
    return mobileRegisterSecurity(
      { ...config.auth.creds, ...registrationOptions },
      config.options,
    );
  };
  const requestRegistrationClientLog = async (registrationOptions) => {
    registrationOptions = registrationOptions || config.auth.creds.registration;
    if (!validRegistrationOptions(registrationOptions)) {
      throw new Error("Invalid registration options");
    }
    sock.authState.creds.registration = registrationOptions;
    sock.ev.emit("creds.update", sock.authState.creds);
    return mobileRegisterClientLog(
      { ...config.auth.creds, ...registrationOptions },
      config.options,
    );
  };

  const requestRegistrationConsent = async (registrationOptions) => {
    registrationOptions = registrationOptions || config.auth.creds.registration;
    if (!validRegistrationOptions(registrationOptions)) {
      throw new Error("Invalid registration options");
    }
  ...
  return {
    ...
    requestRegistrationAbprop,
    requestRegistrationSecurity,
    requestRegistrationClientLog,
...
async function mobileRegisterAbprop(params, fetchOptions) {
...
async function mobileRegisterSecurity(params, fetchOptions, wipeToken = null) {
  return mobileRegisterFetch("/security", {
    params: {
      ...registrationParams(params),
      reset: "wipe",
...
async function mobileRegisterClientLog(params, fetchOptions) {
  return mobileRegisterFetch("/client_log", {
    params: registrationParams(params),
    ...fetchOptions,
  });
}
async function mobileRegisterConsent(params, fetchOptions) {
  const years = [
    "1997",
    "1998",
    "1999",
  ...
  return mobileRegisterFetch("/consent", {
    params: {
      ...registrationParams(params),
      context: "dob",
      dob: randomYear,
