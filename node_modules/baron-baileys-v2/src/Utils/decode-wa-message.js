...
const extractAddressingContext = (stanza) => {
  ...
  const addressingMode =
    ...
    ((sender === null || sender === void 0 ? void 0 : sender.endsWith("lid"))
      ? "lid"
      : "pn");
...
function decodeMessageNode(stanza, meId, meLid) {
  ...
  const key = {
    remoteJid: chatId,
    remoteJidAlt: !(0, WABinary_1.isJidGroup)(chatId)
      ...
      if (Array.isArray(stanza.content)) {
        for (const { tag, attrs, content } of stanza.content) {
          if (tag === "verified_name" && content instanceof Uint8Array) {
            const cert =
              index_js_1.proto.VerifiedNameCertificate.decode(content);
            const details =
              index_js_1.proto.VerifiedNameCertificate.Details.decode(
                cert.details,
              );
          ...
          try {
            ...
            let msg = index_js_1.proto.Message.decode(
              e2eType !== "plaintext"
...
function isSessionRecordError(error) {
  const errorMessage =
    ...
    "";
  return exports.DECRYPTION_RETRY_CONFIG.sessionRecordErrors.some(
