...
const prepareWAMessageMedia = async (message, options) => {
  ...
  const isNewsletter =
    !!options.jid && (0, WABinary_1.isJidNewsletter)(options.jid);
  if (isNewsletter) {
    logger === null || logger === void 0
      ? void 0
      : logger.info(
    ...
    const { filePath, fileSha256, fileLength } = await (0,
    messages_media_1.getRawMediaUploadData)(
      uploadData.media,
    ...
    (async () => {
      try {
        if (requiresThumbnailComputation) {
          ...
          messages_media_1.generateThumbnail)(
            originalFilePath,
            mediaType,
            options,
          );
...
const generateForwardMessageContent = (message, forceForward) => {
  ...
  content = index_js_1.proto.Message.decode(
    index_js_1.proto.Message.encode(content).finish(),
  );
  let key = Object.keys(content)[0];
  let score =
    ((_b =
      (_a = content === null || content === void 0 ? void 0 : content[key]) ===
        null || _a === void 0
        ? void 0
...
const generateWAMessageContent = async (message, options) => {
  ...
  if ("text" in message) {
    ...
    if (contactLen === 1) {
      m.contactMessage = Types_1.WAProto.Message.ContactMessage.create(
        message.contacts.contacts[0],
      );
    } else {
      m.contactsArrayMessage =
        Types_1.WAProto.Message.ContactsArrayMessage.create(message.contacts);
    }
  } else if ("location" in message) {
    m.locationMessage = Types_1.WAProto.Message.LocationMessage.create(
      message.location,
    );
    ...
    m.reactionMessage = Types_1.WAProto.Message.ReactionMessage.create(
      message.react,
    );
          ...
          dispatcher:
            (_a =
              options === null || options === void 0
                ? void 0
                : options.options) === null || _a === void 0
              ? void 0
              : _a.dispatcher,
        });
        if (resp.ok) {
    ...
    switch (message.type) {
      ...
      case "plain":
        m.buttonsResponseMessage = {
          ...
          selectedDisplayText: message.buttonReply.displayText,
          type: index_js_1.proto.Message.ButtonsResponseMessage.Type
            .DISPLAY_TEXT,
        };
        break;
    }
    ...
    m.messageContextInfo = {
      // encKey
      messageSecret:
        message.event.messageSecret || (0, crypto_1.randomBytes)(32),
    };
    ...
    m.eventMessage.endTime = message.event.endDate
      ...
      : undefined;
    m.eventMessage.isCanceled =
      (_b = message.event.isCancelled) !== null && _b !== void 0 ? _b : false;
    m.eventMessage.extraGuestsAllowed = message.event.extraGuestsAllowed;
    m.eventMessage.isScheduleCall =
      (_c = message.event.isScheduleCall) !== null && _c !== void 0
        ? _c
        : false;
    m.eventMessage.location = message.event.location;
  } else if ("poll" in message) {
    (_e = message.poll).selectableCount || (_e.selectableCount = 0);
    (_f = message.poll).toAnnouncementGroup || (_f.toAnnouncementGroup = false);
...
const generateWAMessageFromContent = (jid, message, options) => {
  ...
  if (quoted && !(0, WABinary_1.isJidNewsletter)(jid)) {
    ...
    quotedMsg = index_js_1.proto.Message.create({
    ...
    const contextInfo =
      ("contextInfo" in innerMessage[key] &&
        ((_a = innerMessage[key]) === null || _a === void 0
          ? void 0
          : _a.contextInfo)) ||
      {};
  ...
  const messageJSON = {
    ...
    participant:
      ...
      (0, WABinary_1.isJidStatusBroadcast)(jid)
        ? userJid
        : undefined, // TODO: Add support for LIDs
...
exports.getAggregateResponsesInEventMessage =
...
const downloadMediaMessage = async (message, type, options, ctx) => {
  const result = await downloadMsg().catch(async (error) => {
    if (
      ctx &&
      typeof (error === null || error === void 0 ? void 0 : error.status) ===
        "number" && // treat errors with status as HTTP failures requiring reupload
