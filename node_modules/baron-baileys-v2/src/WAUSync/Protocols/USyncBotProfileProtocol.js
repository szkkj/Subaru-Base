...
class USyncBotProfileProtocol {
  ...
  parser(node) {
    ...
    const commandsNode = (0, WABinary_1.getBinaryNodeChild)(
      profile,
      "commands",
    );
    ...
    const commands = [];
    const prompts = [];
    for (const command of (0, WABinary_1.getBinaryNodeChildren)(
      commandsNode,
      "command",
    )) {
      commands.push({
    ...
    for (const prompt of (0, WABinary_1.getBinaryNodeChildren)(
      promptsNode,
      "prompt",
    )) {
      prompts.push(
    ...
    return {
      ...
      attributes: (0, WABinary_1.getBinaryNodeChildString)(
        profile,
        "attributes",
      ),
      description: (0, WABinary_1.getBinaryNodeChildString)(
