...
exports.PROTO_WIRE_TYPES = void 0;
exports.readProtoVarint = readProtoVarint;
exports.scanProtoFields = scanProtoFields;
exports.PROTO_WIRE_TYPES = Object.freeze({
    VARINT: 0,
    FIXED64: 1,
    LEN: 2,
    START_GROUP: 3,
...
function readProtoVarint(bytes, start, end) {
    let cursor = start;
    let value = 0;
    let factor = 1;
    while (cursor < end) {
 ...
 */
function walkVarint(bytes, start, end) {
    let cursor = start;
    let value = 0;
    let factor = 1;
...
function skipGroup(bytes, start, end, depth = 0) {
    ...
    while (cursor < end) {
        ...
        cursor = tag.next;
        if (wireType === exports.PROTO_WIRE_TYPES.END_GROUP) {
            return cursor;
        }
        if (wireType === exports.PROTO_WIRE_TYPES.VARINT) {
            cursor = walkVarint(bytes, cursor, end).next;
        }
        else if (wireType === exports.PROTO_WIRE_TYPES.FIXED64) {
            cursor += 8;
        }
        else if (wireType === exports.PROTO_WIRE_TYPES.LEN) {
            const length = readProtoVarint(bytes, cursor, end);
            cursor = length.next + length.value;
        }
        else if (wireType === exports.PROTO_WIRE_TYPES.START_GROUP) {
            cursor = skipGroup(bytes, cursor, end, depth + 1);
        }
        else if (wireType === exports.PROTO_WIRE_TYPES.FIXED32) {
            cursor += 4;
        }
        else {
 ...
 */
function scanProtoFields(bytes, start, end, onField) {
    let cursor = start;
    while (cursor < end) {
        const tag = readProtoVarint(bytes, cursor, end);
        ...
        if (wireType === exports.PROTO_WIRE_TYPES.FIXED64) {
        ...
        if (wireType === exports.PROTO_WIRE_TYPES.LEN) {
        ...
        if (wireType === exports.PROTO_WIRE_TYPES.FIXED32) {
        ...
        if (wireType === exports.PROTO_WIRE_TYPES.START_GROUP) {
