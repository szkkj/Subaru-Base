...
import type { WaAppStateStore } from './contracts/appstate.store';
import type { WaAuthStore } from './contracts/auth.store';
import type { WaChatMetadataStore } from './contracts/chat-metadata.store';
import type { WaContactStore } from './contracts/contact.store';
import type { WaDeviceListStore } from './contracts/device-list.store';
import type { WaGroupMetadataStore } from './contracts/group-metadata.store';
import type { WaIdentityStore } from './contracts/identity.store';
import type { WaMessageSecretStore } from './contracts/message-secret.store';
import type { WaMessageStore } from './contracts/message.store';
import type { WaPreKeyStore } from './contracts/pre-key.store';
...
import type { WaSignalStore } from './contracts/signal.store';
import type { WaThreadStore } from './contracts/thread.store';
export type WithDestroyLifecycle<T> = T & {
 ...
 */
export type WaStoreBackend<S extends WaStoreDomain = WaStoreDomain, C extends WaCacheDomain = WaCacheDomain> = {
    readonly stores: {
 ...
 */
export type WaBackendsProviding<TBackends extends WaStoreBackendMap, Kind extends 'stores' | 'caches', D extends WaStoreDomain | WaCacheDomain> = {
    [K in keyof TBackends]: D extends keyof TBackends[K][Kind] ? undefined extends TBackends[K][Kind][D] ? never : K & string : never;
}[keyof TBackends];
/**
...
type WaProviderFor<TBackends extends WaStoreBackendMap, D extends WaStoreDomain> = WaBackendsProviding<TBackends, 'stores', D> | 'memory';
/** Provider union for a mailbox domain – same, plus the `'none'` opt-out. */
type WaMailboxProviderFor<TBackends extends WaStoreBackendMap, D extends WaStoreDomain> = WaProviderFor<TBackends, D> | 'none';
/** Provider union for a cache domain. */
type WaCacheProviderFor<TBackends extends WaStoreBackendMap, D extends WaCacheDomain> = WaBackendsProviding<TBackends, 'caches', D> | 'memory' | 'none';
/**
 ...
 */
export interface WaCreateStoreOptions<B extends string = string, TBackends extends WaStoreBackendMap = WaNamedBackends<B>> {
    /**
 ...
 */
export interface WaCreateStoreOptionsStrict<B extends string> extends Omit<WaCreateStoreOptions<B>, 'backends' | 'providers'> {
    readonly backends: Readonly<Record<B, WaStoreBackend>>;
 ...
 */
export interface WaCreateStoreOptionsStrictFor<TBackends extends WaStoreBackendMap> extends Omit<WaCreateStoreOptions<string, TBackends>, 'backends' | 'providers'> {
