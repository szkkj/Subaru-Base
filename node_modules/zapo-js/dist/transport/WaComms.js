...
class WaComms {
    constructor(config, logger = new ConsoleLogger_1.ConsoleLogger('info')) {
        ...
        this.config = Object.freeze({
            ...routedSocketConfig,
            connectTimeoutMs: routedSocketConfig.connectTimeoutMs ?? 10000,
            reconnectIntervalMs: routedSocketConfig.reconnectIntervalMs ?? 2000,
            timeoutIntervalMs: routedSocketConfig.timeoutIntervalMs ?? 10000
        });
        this.logger = logger;
        this.socket = new WaWebSocket_1.WaWebSocket({
            url: this.config.url,
        ...
        this.socket.setHandlers({
            onOpen: async () => {
                try {
                    ...
                    this.logger.error('noise handshake failed', { message: (0, primitives_1.toError)(error).message });
    ...
    applyRoutingInfoToSocketConfig(config) {
        ...
        return {
            ...
            urls: config.urls
                ...
                : config.url
                    ? undefined
                    : constants_1.WA_DEFAULTS.CHAT_SOCKET_URLS.map((entry) => appendEd(entry))
        };
    }
    ...
    async waitForConnection(timeoutMs = this.config.connectTimeoutMs) {
        if (!this.started) {
    ...
    async closeSocketAndPreventRetry() {
    ...
    async openSocket(isReconnect) {
        ...
        if (isReconnect) {
            ...
            if (this.config.maxReconnectAttempts !== null &&
                ...
                this.started = false;
                this.drainWaiters((waiter) => waiter.reject(new Error('max reconnect attempts reached')));
                return;
            }
        }
        try {
    ...
    async onSocketOpened() {
        this.logger.debug('comms socket opened, starting noise session');
        const noiseConfig = this.buildNoiseConfigForAttempt();
        const session = new WaNoiseSession_1.WaNoiseSession(async (wire) => this.socket.send(wire), this.logger);
        ...
        try {
            ...
            if (this.usedResumeHandshake) {
                ...
                this.logger.warn('noise resume handshake failed, next attempt may fallback to full', {
                    ...
                    threshold: constants_1.WA_DEFAULTS.NOISE_RESUME_FAILURES_BEFORE_FULL_HANDSHAKE
                });
            }
            this.noiseSession = null;
            throw error;
    ...
    tryQueuePendingFrames(frames, source) {
        if (frames.length === 0) {
            return true;
        }
        let incomingBytes = 0;
    ...
    schedulePendingFramesOverflowClose() {
    ...
    buildNoiseConfigForAttempt() {
        const hasServerStaticKey = this.config.noise.serverStaticKey?.byteLength === 32;
        if (hasServerStaticKey &&
            this.resumeHandshakeFailures < constants_1.WA_DEFAULTS.NOISE_RESUME_FAILURES_BEFORE_FULL_HANDSHAKE) {
