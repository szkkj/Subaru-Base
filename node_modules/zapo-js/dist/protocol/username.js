...
exports.WA_USERNAME_VALIDATION_ERRORS = exports.WA_USERNAME_LIMITS = void 0;
exports.validateUsernameLocally = validateUsernameLocally;
exports.isValidUsername = isValidUsername;
exports.isUsernameKey = isUsernameKey;
exports.normalizeUsername = normalizeUsername;
exports.displayUsername = displayUsername;
exports.splitUsernameHandle = splitUsernameHandle;
exports.parseUsernameHandle = parseUsernameHandle;
exports.WA_USERNAME_LIMITS = Object.freeze({
    MIN_LENGTH: 3,
    MAX_LENGTH: 35,
    KEY_LENGTH: 4
});
exports.WA_USERNAME_VALIDATION_ERRORS = Object.freeze({
...
const VALID_VALIDATION = Object.freeze({ isValid: true });
...
function validateUsernameLocally(username) {
    for (let index = 0; index < username.length; index += 1) {
        if (!isAllowedCode(username.charCodeAt(index))) {
            return invalid(exports.WA_USERNAME_VALIDATION_ERRORS.INVALID_CHARACTER);
        }
    }
    if (username.length < exports.WA_USERNAME_LIMITS.MIN_LENGTH ||
        username.length > exports.WA_USERNAME_LIMITS.MAX_LENGTH) {
        return invalid(exports.WA_USERNAME_VALIDATION_ERRORS.INVALID_LENGTH);
    }
    let hasLetter = false;
    for (let index = 0; index < username.length; index += 1) {
    ...
    if (!hasLetter)
        return invalid(exports.WA_USERNAME_VALIDATION_ERRORS.INVALID_NO_LETTERS);
    if (username.startsWith('.') || username.endsWith('.') || username.includes('..')) {
        return invalid(exports.WA_USERNAME_VALIDATION_ERRORS.INVALID_PERIODS);
    }
    const lowered = username.toLowerCase();
    if (lowered.startsWith('www.')) {
        return invalid(exports.WA_USERNAME_VALIDATION_ERRORS.INVALID_WWW_PREFIX);
    }
    for (let index = 0; index < DOMAIN_SUFFIXES.length; index += 1) {
        if (lowered.endsWith(DOMAIN_SUFFIXES[index])) {
            return invalid(exports.WA_USERNAME_VALIDATION_ERRORS.INVALID_DOMAIN_SUFFIX);
        }
    }
    for (let index = 0; index < RESERVED_WORDS.length; index += 1) {
        if (lowered.includes(RESERVED_WORDS[index])) {
            return invalid(exports.WA_USERNAME_VALIDATION_ERRORS.INVALID_WORD);
        }
    }
...
function isUsernameKey(key) {
    if (key.length !== exports.WA_USERNAME_LIMITS.KEY_LENGTH)
        return false;
 ...
 */
function splitUsernameHandle(input) {
    const withoutPrefix = normalizeUsername(input.trim());
    const colonIndex = withoutPrefix.indexOf(':');
    if (colonIndex === -1)
        return { username: withoutPrefix, usernameKey: null };
    return {
...
function parseUsernameHandle(input) {
    const { username, usernameKey } = splitUsernameHandle(input);
    if (!isValidUsername(username))
        return null;
    if (usernameKey !== null && !isUsernameKey(usernameKey))
        return null;
