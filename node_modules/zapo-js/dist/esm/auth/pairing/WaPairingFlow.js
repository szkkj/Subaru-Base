...
import { buildAckNode, buildIqErrorNode, buildIqResultNode } from '../../transport/node/builders/global.js';
...
export class WaPairingFlow {
    ...
    async completePairSuccess(iqNode, pairSuccessNode) {
        ...
        const deviceIdentityBytes = decodeNodeContentUtf8OrBytes(deviceIdentityNode.content, 'pair-success.device-identity');
    ...
    async sendPairSuccessValidationError(iqNode) {
        try {
            await this.opts.socket.sendNode(buildIqErrorNode(iqNode, { code: 401, text: 'not-authorized' }));
        }
        catch (error) {
