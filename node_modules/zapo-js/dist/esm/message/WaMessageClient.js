...
export class WaMessageClient {
     ...
     */
    async publishNode(node, options = {}) {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
            try {
                ...
                if (isNegativeAckNode(ackNode)) {
                    throw new MessagePublishNackError(`negative publish ack: ${describeAckNode(ackNode)}`, isRetryableNegativeAck(ackNode));
                }
                if (attempt > 1) {
                ...
                const nackRetryable = error instanceof MessagePublishNackError ? error.retryable : false;
    ...
    async publishEncrypted(input, options = {}) {
