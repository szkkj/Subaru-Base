...
import { isGroupOrBroadcastJid, isLidJid, isNewsletterJid, isUserJid, toUserJid } from '../../protocol/jid.js';
 ...
 */
export function collectUniqueUserJids(...candidates) {
    const out = [];
    const seen = new Set();
 ...
 */
export function resolveAddonParentSenderFromKey(targetMessageKey, chatIsGroup) {
...
export function buildAddonSenderPairs(input) {
    ...
    addPair(parents.find((jid) => isLidJid(jid)), modifiers.find((jid) => isLidJid(jid)));
    addPair(parents.find((jid) => isUserJid(jid)), modifiers.find((jid) => isUserJid(jid)));
