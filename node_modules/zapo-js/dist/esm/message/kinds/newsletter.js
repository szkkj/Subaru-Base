...
import { WA_EDIT_ATTRS } from '../../protocol/message.js';
...
export function processNewsletterLiveUpdates(notification, options) {
...
export function processIncomingNewsletterMessage(node, options) {
    ...
    if (reactionsEnvelope) {
        ...
        if (reactionNode) {
            const revoked = editAttr === WA_EDIT_ATTRS.SENDER_REVOKE || messageType === 'reaction_revoke';
            emitUpdate(node, options, {
                kind: 'reaction',
                isSender: node.attrs.is_sender === 'true',
                revoked,
...
function decodePlaintext(node, options) {
    ...
    try {
        const plaintext = decodeNodeContentBase64OrBytes(plaintextNode.content, 'newsletter.plaintext');
