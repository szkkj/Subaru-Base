import { WA_BUSINESS_HOURS_MODES, WA_DEFAULTS, WA_IQ_TYPES, WA_NODE_TAGS, WA_XMLNS } from '../../../protocol/constants.js';
...
const VALID_BUSINESS_HOURS_MODES = new Set(Object.values(WA_BUSINESS_HOURS_MODES));
function buildBusinessHoursNode(hours) {
    ...
    for (let i = 0; i < hours.config.length; i += 1) {
        ...
        if (!VALID_BUSINESS_HOURS_MODES.has(entry.mode)) {
            throw new Error(`invalid business hours mode '${entry.mode}' (expected one of ${Object.values(WA_BUSINESS_HOURS_MODES).join(', ')}; closed days must be omitted)`);
        }
        const attrs = {
...
export function buildGetBusinessUsyncQueryNode() {
    return {
...
export function buildCoverPhotoIq(input) {
    const attrs = input.op === 'update'
