...
const WaMediaCrypto_1 = require("../crypto/WaMediaCrypto");
...
class WaMediaTransferClient {
    ...
    async downloadBytes(request) {
        ...
        return (0, bytes_1.readAllBytes)(response.body, {
    ...
    async uploadStream(request) {
        ...
        const { urls, headers, timeoutMs } = this.resolveTransferRequest(request, {
            ...
            'content-length': request.contentLength !== null && request.contentLength !== undefined
    ...
    async uploadEncrypted(request) {
        ...
        const mediaKey = request.mediaKey ?? (await WaMediaCrypto_1.WaMediaCrypto.generateMediaKey());
        ...
        try {
            ...
            await prepared.cleanup((0, primitives_1.toError)(error));
            throw error;
        }
    ...
    async downloadAndDecrypt(request) {
        ...
        try {
            const [plaintext] = await Promise.all([
                (0, bytes_1.readAllBytes)(decrypted.plaintext, {
                    maxBytes: request.maxBytes ?? this.defaultMaxReadBytes
                }),
                decrypted.metadata
            ]);
            ...
            decrypted.plaintext.destroy((0, primitives_1.toError)(error));
            throw error;
        }
    }
    /**
     ...
     */
    async downloadAndDecryptStream(request) {
        const response = await this.downloadStream(request);
        await this.assertSuccessfulResponse(response);
        const body = this.requireResponseBody(response);
        const decrypted = await WaMediaCrypto_1.WaMediaCrypto.decryptReadable(body, {
            mediaType: request.mediaType,
    ...
    async readResponseBytes(response, maxBytes) {
        if (!response.body) {
            return bytes_1.EMPTY_BYTES;
        }
    ...
    async prepareEncryptedUpload(request, mediaKey) {
        if (request.plaintext instanceof Uint8Array) {
            const encrypted = await WaMediaCrypto_1.WaMediaCrypto.encryptBytes(request.mediaType, mediaKey, request.plaintext);
            return {
        ...
        const prepared = await WaMediaCrypto_1.WaMediaCrypto.encryptReadable(request.mediaType, mediaKey, request.plaintext, { expectedFileSize: request.contentLength ?? undefined });
        return {
            body: prepared.encrypted,
            contentLength: request.contentLength !== null && request.contentLength !== undefined
                ? WaMediaCrypto_1.WaMediaCrypto.encryptedLength(request.contentLength)
                : undefined,
    ...
    async assertSuccessfulResponse(response) {
        if (response.ok) {
            return;
        }
    ...
    createAbortContext(timeoutMs, externalSignal) {
...
exports.WaMediaTransferClient = WaMediaTransferClient;
