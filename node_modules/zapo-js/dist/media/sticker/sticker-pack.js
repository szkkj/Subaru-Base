...
exports.createStickerPackZipStream = createStickerPackZipStream;
...
async function loadEntryBytes(source) {
    if (typeof source === 'string') {
        const buf = await (0, promises_1.readFile)(source);
        return new Uint8Array(buf.buffer, buf.byteOffset, buf.byteLength);
    }
    return source;
}
function buildLocalHeader(name, crc, dataLength) {
...
function buildCentralDirectory(entries) {
    let totalSize = 0;
    for (const entry of entries)
        totalSize += CENTRAL_HEADER_SIZE + entry.name.byteLength;
...
function createStickerPackZipStream(entries) {
    validateEntries(entries);
    return node_stream_1.Readable.from(zipChunks(entries));
}
async function* zipChunks(entries) {
    ...
    for (const entry of entries) {
        ...
        const name = bytes_1.TEXT_ENCODER.encode(entry.fileName);
