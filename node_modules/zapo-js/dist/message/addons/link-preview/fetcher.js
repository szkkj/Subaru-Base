...
const DEFAULT_HTML_BYTES = 65536;
const DEFAULT_THUMBNAIL_BYTES = 262144;
const INLINE_THUMBNAIL_MAX_BYTES = 64 * 1024;
const MAX_REDIRECTS = 5;
function createDefaultLinkPreviewFetcher(options) {
    ...
    const agent = (0, proxy_1.toProxyAgent)(options.proxy);
    ...
    return {
        async fetch(input) {
            ...
            const minimal = {
                ...
                previewType: _proto_1.proto.Message.ExtendedTextMessage.PreviewType.NONE
            };
            let html;
            try {
            ...
            return {
                ...
                previewType: parsed.hasVideo
                    ? _proto_1.proto.Message.ExtendedTextMessage.PreviewType.VIDEO
                    : _proto_1.proto.Message.ExtendedTextMessage.PreviewType.NONE,
...
function parseHtmlMeta(html, base) {
    ...
    const ogImage = extractMetaContent(head, 'property', 'og:image:secure_url') ??
    ...
    const ogImageWidth = (0, primitives_1.parseOptionalInt)(extractMetaContent(head, 'property', 'og:image:width'));
    const ogImageHeight = (0, primitives_1.parseOptionalInt)(extractMetaContent(head, 'property', 'og:image:height'));
    const ogVideo = extractMetaContent(head, 'property', 'og:video:secure_url') ??
...
function extractMetaContent(html, attr, key) {
    ...
    const re1 = new RegExp(`<meta\\b[^>]*?\\b${attr}\\s*=\\s*["']${escaped}["'][^>]*?\\bcontent\\s*=\\s*["']([^"']*)["']`, 'i');
    const re2 = new RegExp(`<meta\\b[^>]*?\\bcontent\\s*=\\s*["']([^"']*)["'][^>]*?\\b${attr}\\s*=\\s*["']${escaped}["']`, 'i');
...
async function fetchHtml(url, opts) {
    ...
    return bytes_1.TEXT_DECODER.decode(result.bytes);
}
async function fetchThumbnail(url, opts) {
    let parsed;
    try {
        parsed = new URL(url);
    }
...
async function httpGetWithRedirects(opts) {
    const streamResult = await httpGetStreamWithRedirects({
        url: opts.url,
        mediaTransfer: opts.mediaTransfer,
        agent: opts.agent,
...
async function httpGetStreamWithRedirects(opts) {
    ...
    for (let redirects = 0; redirects <= MAX_REDIRECTS; redirects++) {
        ...
        const contentLength = (0, primitives_1.parseOptionalInt)(contentLengthHeader);
        return {
            status: response.status,
            contentType,
            contentLength,
...
function isPrivateHost(hostname) {
    ...
    if (bare === '::1' || bare === '0:0:0:0:0:0:0:1' || bare === '::')
        return true;
    if (bare.startsWith('::ffff:')) {
        return isPrivateHost(bare.slice('::ffff:'.length));
    }
    if (PRIVATE_IPV4.some((re) => re.test(bare)))
        return true;
