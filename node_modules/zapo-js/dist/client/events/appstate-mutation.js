...
function parseAppStateMutationEvent(mutation) {
    ...
    const resolved = (0, _appstate_spec_1.getAppstateSchemaByActionName)(parts[0]);
    if (!resolved) {
        return null;
    }
    const indexArgs = (0, _appstate_spec_1.decodeIndexArgsFromSchema)(resolved.schema, parts);
    if (!indexArgs) {
        return null;
    }
    ...
    for (const [fieldPath, enumPath] of Object.entries(enumFields)) {
        applyDecodeAtPath(data, fieldPath.split('.'), (raw) => {
            if (typeof raw !== 'number')
                return raw;
            const name = (0, _appstate_spec_1.decodeEnumValue)(enumPath, raw);
            return name ?? raw;
        });
    }
}
